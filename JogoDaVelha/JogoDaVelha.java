...
class JogoDaVelha {
					...
					if(turn==2 && vsCpu) choice=cpuTurn(turn);
						...
						do {
							...
							if(choice<0 || choice>8) 
								util.imprimirTexto("Tecla invalida! Tente o teclado numerico da direita (1-9)\n\n");
						}
	...
	public static int whoWin() {
		for(int n=1;n<=2;n++)
			for(int n2=0;n2<3;n2++) {
				if((check(n2*3)==n && check(n2*3+1)==n && check(n2*3+2)==n) ||
					(check(n2)==n && check(3+n2)==n && check(6+n2)==n) ||
					(n2<2 && (check(n2*2)==n && check(4)==n && check(8-n2*2)==n))) return n;
			}
		return 0;
	}
	...
	public static void desenhaCampo() {
		...
		for(int y=2;y>=0;y--) {
			for(int x=0;x<3;x++) 
				util.imprimirTexto("["+(campos[y][x]==0?" ":campos[y][x]==1?"O":"X")+"]");
			util.imprimirTexto();
		}
	...
	public static boolean check(int pos1,int pos2,int pos3,int val) {
		return (campos[(int)(pos1/3)][pos1%3]==val &&
				campos[(int)(pos2/3)][pos2%3]==val &&
				campos[(int)(pos3/3)][pos3%3]==0)?true:false;
	}

	public static int check(int pos1,int pos2,int pos3) {
		return (campos[(int)(pos1/3)][pos1%3]!=0 &&
				campos[(int)(pos2/3)][pos2%3]==0 &&
				campos[(int)(pos3/3)][pos3%3]==0)?util.rnd(0,1)==0?pos2:pos3:-1;
	}

	public static int check(int pos) { return campos[(int)(pos/3)][pos%3]; }
	...
	public static int cpuTurn(int turn) {
		...
		for(int t=turn,z=0;choice==-1 && z<2;z++,t=t==1?2:1) {
			...
			for(int n=0;choice==-1 && n<3;n++)
				for(int n2=0;choice==-1 && n2+3<pos[2].length;n2+=3) {
		...
		if(choice==-1) {
			...
			else { do choice=util.rnd(0,8); while(check(choice)!=0); }
		}
		return choice;
	}
}