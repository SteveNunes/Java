...
public class JogoDaForca {
  ...
  public static void rodadaAtual() {
    ...
    while (jogador.getVidas()>0&&!jogo.palavraFoiRevelada()) {
      ...
      if (jogo.letraJaUtilizada(letra)) util.imprimirTexto("A letra "+letra+" ja foi utilizada! Tente outra letra!");
      else {
  ...
  public static void fimDeRodada() {
    if (jogo.palavraFoiRevelada()) {
      ...
      if (!palavras.getTemaAtual(jogo.getNivelAtual()).equals("Indisponivel")) util.imprimirTexto("Vamos para o proximo level!");
      util.imprimirTexto("\n\n");
    }
  }

  public static void fimDeJogo() {
    if (jogador.getVidas()==0) util.imprimirTexto("Fim de jogo, pois voce ficou sem vidas!\n"+"A palavra atual era: "+jogo.getPalavraAtual()+"");
    else if (palavras.getTemaAtual(jogo.getNivelAtual()).equals("Indisponivel")) util.imprimirTexto("Fim de jogo, pois nao temos mais niveis disponiveis!");
