...
public class Sporcle {
	...
	public static void rodadaAtual() {
		...
		while (util.getTempoLimite() > 0 && jogo.getPalavrasRestantes() > 0) {
			...
			if (jogo.palavraJaUtilizada(palavra)) 
				util.imprimirTexto("A palavra " + palavra + " ja foi utilizada! Tente outra palavra!");
			else {
	...
	public static void fimDeRodada() {
		if (jogo.getPalavrasRestantes() == 0) {
			...
			if (palavras.getTemaAtual(jogo.getNivelAtual()) != "Indisponivel") 
	...
	public static void fimDeJogo() {
		if (util.getTempoLimite() <= 0) {
			...
			int quebraLinha = 1;
			for(String i : palavras.listaDePalavras(jogo.getNivelAtual()))
				if(!jogo.palavraJaUtilizada(i)) util.imprimirTexto(i + (quebraLinha++ % 10 == 0 ? "\n" : " "));
		}
		else if (palavras.getTemaAtual(jogo.getNivelAtual()) != "Indisponivel") 
