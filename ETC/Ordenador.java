...
public class Ordenador {
  ...
  public static String[] ordenar(String[] nonOrdered,int reverse) {
    ...
    while (pos<ordered.length) {
      m="";
      for (String i : nonOrdered) if (!i.equals("")&&((reverse==0&&i.compareTo(m)>0)||(reverse==1&&i.compareTo(m)<0))) m=i;
      for (int n=0; n<nonOrdered.length; n++) if (!nonOrdered[n].equals("")&&((reverse==0&&nonOrdered[n].compareTo(m)<=0)||(reverse==1&&nonOrdered[n].compareTo(m)>=0))) {
        m=nonOrdered[n];
        del=n;
      }
      ordered[pos++]=m;
  ...
  public static String[] ordenar(String[] nonOrdered) {
    return ordenar(nonOrdered,0);
  }
}
