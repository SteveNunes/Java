...
public class Token {
  ...
  public Token() {
    totalTokenized=backuptotalTokenized=0;
    defaultDelimiter=" ";
    caseIgnore=false;
  }

  /**
   * Apenas para uso privado. Aplica um tokenize() com os parametros passados, porém salva os parametros atuaexists da classe para serem recuperados posteriormente.
   */
  private void saveAndTokenize(String texto,String delimitador) {
    backupToken=token;
    int backuptotalTokenized=totalTokenized;
    tokenize(texto,delimitador);
  }

  /**
   * Apenas para uso privado em conjunto com saveAndTokenize(). Recupera os parametros originaexists salvos pelo metodo saveAndTokenize()
   */
  private void loadSavedTokens() {
    if (backuptotalTokenized>0) {
      token=backupToken;
      totalTokenized=backuptotalTokenized;
    }
  }
  ...
  /**
   * Quebra um texto em tokens, colocando cada token na matriz de String token[] e define o valor do atributo 'totalTokenized' para o total de tokens obtidos.
   *
   ...
   */
  public void tokenize(String texto) {
    tokenize(texto,defaultDelimiter);
  }
  ...
  /**
   * Sobrecarga que permite nao especificar o delimitador. Nesse caso, será usado o delimitador padrão como parametro.
   */
  public int count(String texto) {
    return count(texto,defaultDelimiter);
  }
  ...
  public String get(String texto,int posicaoInicial,int posicaoFinal,String delimitador) {
    ...
    String novaPalavra="";
    for (int n=1; n<=totalTokenized; n++) if (n>=posicaoInicial&&(posicaoFinal==-1||n<=posicaoFinal)) novaPalavra+=(novaPalavra.isEmpty()?token(n):(delimitador+token(n)));
    loadSavedTokens();
    return novaPalavra;
  }
  ...
  public int find(String texto,String token,String delimitador) {
    int found=0;
    saveAndTokenize(texto,delimitador);
    for (int n=1; n<=totalTokenized; n++) if (token(n).equals(token)) found=n;
    loadSavedTokens();
    return found;
  }
  ...
  public String remove(String texto,String ocorrencia,String delimitador) {
    saveAndTokenize(texto,delimitador);
    String novaPalavra="";
    for (int n=1; n<=totalTokenized; n++) if (!token(n).equals(ocorrencia)) novaPalavra+=(novaPalavra.isEmpty()?token(n):(delimitador+token(n)));
    loadSavedTokens();
    return novaPalavra;
  }
  ...
  /**
   * Insere o token especificado no texto, na posição especifcada. O token atual na posição especificada é deslocado para direita, jutamente com os tokens sequenciaexists, e o novo token é
   * inserido na posição especificada.
   *
  ...
  /**
   * Insere o token especificado no texto, na posição especifcada. O token atual na posição especificada é substituido pelo novo token.
   *
   * @param texto       - O texto da qual se quer inserir o token
   * @param token       - O token a ser inserido
   * @param delimitador - Texto ou letra que servirá como delimitador dos tokens
  ...
  /**
   * Substitui o token especificado por outro token. Essa substituição poderá ser aplicada em posições especificas ou no texto inteiro.
   *
   ...
   * @param posicao      - Se for 0, aplica a substituicao em TODAS as ocorrencias encontradas. Se for 1, aplica apenas na primeira ocorrencia, Se for 2, aplica apenas na segunda ocorrencia, e assim
   *                     em diante...
   * @param delimitador  - Texto ou letra que servirá como delimitador dos tokens
   */
  ...
  /**
   * Sobrecarga que permite nao especificar a posição. Nesse caso, a substituição será aplicada apenas a primeira ocorrencia encontrada.
   */
  ...
  /**
   * Sobrecarga que permite nao especificar a posição e o delimitador. Nesse caso, será usado o delimitador padrão como parametro, e a substituição será aplicada apenas a primeira ocorrencia
   * encontrada.
   */
  ...
  /**
   * Sobrecarga que permite nao especificar o parametro 'reverse' Nesse caso, será aplicada a ordem crescente
   */
  public String sort(String texto,String delimitador) {
    return sort(texto,0,defaultDelimiter);
  }

  /**
   * Sobrecarga que permite nao especificar o delimitador. Nesse caso, será usado o delimitador padrão como parametro.
   */
  public String sort(String texto,int reverse) {
    return sort(texto,reverse,defaultDelimiter);
  }

  /**
   * Sobrecarga que permite nao especificar nem o delimitador, nem o parametro 'reverse' Nesse caso, será aplicada a ordem crescente, e será usado o delimitador padrão como parametro.
   */
  public String sort(String texto) {
    return sort(texto,0,defaultDelimiter);
  }
  ...
  public static void main(String args[]) {
    ...
    for (int n=1; n<=token.totalTokenized(); n++) System.out.println("token.token("+n+") = "+token.token(n));
