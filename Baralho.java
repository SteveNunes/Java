class Baralho {
	...
	Carta[] cartasMaioresQue(Valor valorDaCarta) {
		...
		for(Valor valor : Valor.values())
			for(Naipe naipe : Naipe.values())
				if(valor.ordinal() > valorDaCarta.ordinal()) resultado[posicao++] = new Carta(naipe, valor);
		return resultado;
	}
}
...
class Carta {
	private Naipe naipe;
	private Valor valor;
	Carta(Naipe naipe, Valor valor) { this.naipe = naipe; this.valor = valor; }
	public Naipe naipe() { return naipe; }
	public Valor valor() { return valor; }
	public String toString() { return naipe + " " + valor; }
}

enum Naipe { PAUS, COPAS, ESPADA, OURO; }

enum Valor { AS, DOIS, TRES, QUATRO, CINCO, SEIS, SETE, OITO, NOVE, DEZ, VALETE, DAMA, REIS; }